...
const convertEmailHelper = function convertEmailHelper(emailObject) {
  let emailString = '';
  if (_.isString(emailObject.to)) {
    emailString = emailObject.to;
  } else if (_.isObject(emailObject.to)) {
    if (emailObject.to.email && emailObject.to.name) {
      emailString = `${emailObject.to.name} <${emailObject.to.email}>`;
    } else if (emailObject.to.email) {
      emailString = emailObject.to.email;
    }
  }
  return emailString;
};
